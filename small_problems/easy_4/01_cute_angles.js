...
function padZero(str) { return (str.length < 2 ? '0' + str : str); }

function dms(num) {
  var sign = (num < 0 ? '-' : '');
  var numAbs = Math.abs(num);
  var degs = String(Math.floor(numAbs));
  var mins = padZero(String(Math.floor((numAbs - degs) * 60)));
  var secs = padZero(String(Math.floor(((numAbs - degs) * 60 - mins) * 60)));
